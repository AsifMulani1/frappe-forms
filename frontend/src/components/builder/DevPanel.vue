  ...
  <div class="dev-panel">
    <div class="flex items-center justify-between px-4 border-b border-outline-gray-1 h-[52px] shrink-0">
      <div class="flex items-center gap-2">
        ...
        <div class="flex flex-col leading-tight">
          ...
          <span class="text-2xs text-ink-gray-5">{{ linked ? `Web Form → ${form.target_doctype}` : 'New DocType schema' }}</span>
        </div>
      </div>
    ...
    <div class="flex-1 overflow-y-auto p-4">
      ...
      <template v-else-if="tab === 'map'">
        <div class="grid grid-cols-[1fr_auto_1fr] gap-2.5 text-2xs text-ink-gray-5 font-mono pb-2">
        ...
        <div v-for="f in form.fields" :key="f.name"
          ...
          <div class="flex items-center gap-2 min-w-0">
            ...
            <div class="flex flex-col min-w-0">
              ...
              <span class="text-2xs text-ink-gray-5">{{ FT[f.field_type].label }}{{ f.reqd ? ' · required' : '' }}</span>
            </div>
          </div>
          <Icon :name="linked && !f.mapped_field ? 'x' : 'arrow-right'" :size="14" class="text-ink-gray-4" />
          <div class="flex flex-col min-w-0 font-mono text-xs">
      ...
      <template v-else>
        <div class="border border-outline-gray-2 rounded-md bg-surface-base p-3.5 mb-2.5">
          ...
          <div class="border border-outline-gray-1 rounded bg-surface-gray-1 font-mono text-2xs p-2 mt-2" v-if="!linked">
        ...
        <div class="border border-outline-gray-2 rounded-md bg-surface-base p-3.5 mb-2.5">
          ...
          <div class="flex flex-wrap gap-1.5 mt-2">
            <span v-for="f in form.fields" :key="f.name" class="inline-flex items-center gap-1 border border-outline-gray-2 rounded font-mono text-2xs px-1.5 py-0.5">
